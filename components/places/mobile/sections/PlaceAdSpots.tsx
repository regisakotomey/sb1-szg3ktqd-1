...
export default function PlaceAdSpots({ placeId }: PlaceAdSpotsProps) {
    ...
    return (
      <div className="bg-white rounded-lg p-4">
        ...
        <div className="flex gap-2 overflow-x-auto hide-scrollbar">
          {[1, 2, 3].map((i) => (
            <div key={i} className="flex-none w-[120px] aspect-[9/16] bg-gray-200 rounded-lg animate-pulse"></div>
          ))}
        </div>
      </div>
    );
  ...
  return (
    <div className="bg-white rounded-lg p-4">
      <div className="flex items-center justify-between mb-3">
        <h3 className="text-sm font-semibold">Spots publicitaires</h3>
        <button
          onClick={() => router.push(`/content/places/${placeId}/ads`)}
      ...
      >
        ...
        {isLoadingMore && (
          <div className="flex-none w-[120px] aspect-[9/16] bg-gray-200 rounded-lg animate-pulse"></div>
        )}
      </div>
    </div>
  );
